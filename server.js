...
const tankTypes = {
  Tank: { speed: 3, health: 100, damage: 10 },
  Rammer: { speed: 5, health: 70, damage: 5 },
  Sniper: { speed: 2, health: 80, damage: 25 },
};
...
setInterval(()=>{
  ...
  // Simple bot AI: move towards nearest player
...
io.on('connection',socket=>{
  ...
  socket.on('shoot',data=>{
    ...
    bullets.push({x:p.x,y:p.y,vx:Math.cos(angle)*10,vy:Math.sin(angle)*10,damage:tankTypes[p.type].damage});
  });
